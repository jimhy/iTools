...
- **文件搜索**（`/f` 前缀）：三级后端，按可用性自动降级，当前用的是哪一级会在 `/f` 面板顶部**如实显示**
  - **全盘索引（需一次管理员授权）**：直读各固定 NTFS 盘的 MFT 建内存索引，**任意盘任意位置**按文件名子串搜索。本机实测：四盘 733 万条可搜索条目、建索引 17 秒、占用 358 MB、单次查询 28–40 ms（服务端自计时）。索引由一个独立的提权后台进程持有（主程序**不**提权），跟 USN 日志增量更新，可在设置里关闭并释放内存
  - **Windows Search**（未开启全盘索引时）：OLE DB 直连 SystemIndex。⚠ 它只覆盖系统索引范围（默认往往仅 `C:\Users\`），**其它盘通常搜不到**，且 `CONTAINS` 只支持词前缀——搜「报告」搜不到「季度报告.docx」。这正是要做全盘索引的原因
  - **walkdir 兜底**：上面两级都不可用时，递归索引用户的桌面/文档/下载/图片
  - 默认排除 `node_modules`/`target`/`.git`/`WinSxS` 等机器生成目录（本机实测排掉 618 万条），避免结果被构建产物淹没
  - **结果排序**综合四类信号：匹配质量（完全同名 > 从头 > 词首/词尾 > 中间）· 位置（桌面/文档/下载 优先于系统目录，路径越浅越靠前）· 文件类型（文档 > 媒体/源码 > 程序，`.tmp`/`.log`/`.pdb` 等机器产物明确沉底）· 最近修改时间
